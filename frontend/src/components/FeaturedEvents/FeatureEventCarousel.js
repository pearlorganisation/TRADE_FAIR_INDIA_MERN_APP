...
const FeatureEventCarousel = () => {
  ...
  return (
    ...
    >
        {
            Array(6).fill(true).map((item,idx) =>{
                const isPrev = idx === activeIndex - 1;
                const isNext = idx === activeIndex + 1;
                const isActive = idx === activeIndex +1 ;
                return   <SwiperSlide
                 className=''>  <div className={`bg-white h-[30rem] max-w-[25rem] px-3 pt-3 rounded-lg grid grid-rows-[15rem_auto] mx-auto`}>
                ...
                <div className='     divide-y-2 p-2'>
                 <div className='font-medium text-lg'>All Inida Meetup 2023 ft. 50 Cent | Mumbai</div>
                 <div className='font-medium text-sm text-[#00373E]'>
                    <span className='text-xs'>By Bombay inc</span><span className='flex justify-start items-center gap-1'><FaCalendarAlt /> November 25</span><span className='flex justify-start items-center gap-1'> <HiOutlineLocationMarker /> D Y Patil Stadium Raipur</span>
                 </div>
                </div>
              </div></SwiperSlide>
            })
