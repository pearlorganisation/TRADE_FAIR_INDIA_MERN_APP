...
const ExploreByChoice = () => {
  return (
    <div className='container mx-auto min-h-[50dvh] flex flex-col gap-12 justify-center items-center'>
        ...
        <div className='flex items-end flex-wrap w-[60%] gap-10'>
            {
                ["🎉 Weekend","😍 Free","📆 This Month","🌟 Tomorrow","🍭 Today"].map(item =>{
                    return <div className='bg-white font-medium text-2xl flex-grow basis-[15rem] text-center py-3 rounded-3xl cursor-pointer hover:scale-110 hover:shadow-2xl transition-all'>{item}</div>
                })
            }
        </div>
    </div>
