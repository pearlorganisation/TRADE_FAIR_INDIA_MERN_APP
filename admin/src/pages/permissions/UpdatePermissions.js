...
import { updatePermission } from "../../features/actions/permissionActions";
...
const UpdatePermissions = () => {
  ...
  const [isPermissionUpdationApiCalled, setIsPermissionUpdationApiCalled] =
    useState(false);

  // Redirection to Permissions Listing Page after Permission Updation.
  useEffect(() => {
    if (isSuccess && !errorMessage && isPermissionUpdationApiCalled)
      navigate("/permissions");
  }, [navigate, errorMessage, isSuccess, isPermissionUpdationApiCalled]);

  //Showing Error Message in toast
  useEffect(() => {
    errorMessage &&
      isPermissionUpdationApiCalled &&
      toast.error(errorMessage, {
        position: "bottom-center",
      });
  }, [errorMessage, isPermissionUpdationApiCalled]);

  const onSubmit = (formData) => {
    dispatch(updatePermission({ permissionId: state?._id, payload: formData }));
    setIsPermissionUpdationApiCalled(true);
  };
